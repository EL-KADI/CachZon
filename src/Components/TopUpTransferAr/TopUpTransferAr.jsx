...
import "./TopUpTransferAr-module.css";
export default function TopUpTransferAr() {
  ...
  const sendVerificationCode = async () => {
    try {
      ...
      if (!egyptPhoneRegex.test(phoneNumber1)) {
        setErrorMessage("الرجاء إدخال رقم هاتف مصري صحيح.");
        return;
      }
      ...
      setErrorMessage(`فشل إرسال رمز التحقق: ${error.message}`);
    }
  };
  ...
  const verifyCode = async () => {
    try {
      ...
      setErrorMessage(`فشل التحقق من الرمز: ${error.message}`);
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (!showVerificationCodeInput) {
      if (!isValid1 || !isValid2 || areNumbersEqual) {
        setErrorMessage(
          "الرجاء التأكد من أن أرقام الهواتف صحيحة وليست متطابقة."
        );
        return;
      }
  ...
  useEffect(() => {
    if (showVerificationCodeInput && timeLeft > 0) {
      ...
      setErrorMessage("انتهت صلاحية رمز التحقق. الرجاء المحاولة مرة أخرى.");
      setShowVerificationCodeInput(false);
    }
        ...
        >
          {!showVerificationCodeInput && (
            <>
              ...
              <label
                ...
                className="block mb-2 text-sm font-medium mt-10 text-right text-gray-200"
              >
                المستلم
              </label>
              <div className="relative">
              ...
              {areNumbersEqual && (
                <p className="mt-2 text-sm text-red-600 dark:text-red-500">
                  لا يمكن أن تكون أرقام الهواتف متطابقة.
                </p>
              )}
            </>
          )}
          ...
          {showVerificationCodeInput && (
            <>
              ...
              {!isCodeValid && (
                <p className="mt-2 text-sm text-red-600 dark:text-red-500">
                  رمز التحقق غير صحيح. الرجاء إدخال الرمز الصحيح.
                </p>
              )}
          ...
          >
            {showVerificationCodeInput ? "إدخال الرمز" : "إرسال"}
          </button>
